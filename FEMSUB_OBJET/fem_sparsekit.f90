...
CONTAINS
  ...
  SUBROUTINE qs_massvar_M (mesh, ff, ia, ja,  a0)
    !===========================================================
    !  < w, ff _ >
    !  ===>  a0   ! incremental accumulation
    USE Gauss_points
    ...
    REAL(KIND=8) ::  masslm
    REAL(KIND=8), DIMENSION(mesh%gauss%n_w,mesh%gauss%n_w,mesh%gauss%l_G) :: wwprod
    REAL(KIND=8), DIMENSION(mesh%gauss%n_w,mesh%gauss%n_w) :: xx
    REAL(KIND=8), DIMENSION(mesh%gauss%n_w) :: floc
    CALL gauss(mesh)
    jj => mesh%jj
    me => mesh%me
    DO l = 1, l_G
    ...
    DO m = 1, me
       ...
       xx = 0.d0
       DO l = 1, l_G
          masslm = SUM(floc*ww(:,l))*rj(l,m)
          xx = xx + masslm*wwprod(:,:,l)
       END DO
  ...
  SUBROUTINE edge_stab_M(mesh, coeff_visc, ia, ja, aa)
    ...
    INTEGER :: ls, ms, cotei, cotej, p, ni, nj, i, j, type_fe, m1, m2
    ...
    DO ms = 1, mesh%mi
       ...
       m1 = mesh%neighi(1,ms)
       m2 = mesh%neighi(2,ms)
       h2 = (0.5*SUM(mesh%gauss%rj(:,m1)+ mesh%gauss%rj(:,m2))/SUM(mesh%gauss%rji(:,ms)))**2
       h2 = h2*coeff
