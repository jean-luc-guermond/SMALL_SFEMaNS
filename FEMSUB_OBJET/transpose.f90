...
!!$    END DO
  ...
  SUBROUTINE duplicate(temp,mat)
    USE matrix_type
    IMPLICIT NONE
    TYPE(matrice_bloc), INTENT(IN) :: temp
    TYPE(matrice_bloc), INTENT(OUT) :: mat
    ALLOCATE(mat%ia(SIZE(temp%ia)))
    ALLOCATE(mat%ja(SIZE(temp%ja)))
    ALLOCATE(mat%aa(SIZE(temp%aa)))
    mat%ia=temp%ia
    mat%ja=temp%ja
    mat%aa=0.d0
