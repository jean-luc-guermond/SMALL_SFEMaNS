...
CONTAINS
  ...
  SUBROUTINE coupe(mesh,r_0,r_1,n_point,field,file_name,opt_out)
    USE def_type_mesh
    IMPLICIT NONE
    TYPE(mesh_type)                          :: mesh
...
100 FORMAT(2(e11.5,2X))
  ...
  FUNCTION ff(mesh, m,r)
    ...
    REAL(KIND=8) :: x, y, one=1, two=2, four=4, half  = 0.5d0
...
100 FORMAT(3(e11.5,3x),i5)
  ...
  FUNCTION inside(mesh,m,r) 
    ...
    REAL(KIND=8), DIMENSION(2) :: r1, r2, r3, &
         n1, n2, n3 
    REAL(KIND=8) :: s1, s2, s3, epsilon=-1.d-12
    r1 = mesh%rr(:,mesh%jj(1,m))
    r2 = mesh%rr(:,mesh%jj(2,m))
    r3 = mesh%rr(:,mesh%jj(3,m))
    ...
    n1 = pd_vect(r3-r2)
    IF (pd_scal(n1,r2-r1) >0.d0) THEN
       n1 = -n1 !===n1 should point to the interior
    END IF
    s1 = pd_scal(n1, r - r2)

    n2 = pd_vect(r1-r3)
    IF (pd_scal(n2,r3-r2) >0.d0) THEN
       n2 = -n2
    END IF
    s2 = pd_scal(n2, r - r3)

    n3 = pd_vect(r2-r1)
    IF (pd_scal(n3,r1-r3) >0.d0) THEN
       n3 = -n3
    END IF
    s3 = pd_scal(n3, r - r1)
