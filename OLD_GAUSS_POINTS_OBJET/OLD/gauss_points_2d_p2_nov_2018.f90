  ...
  SUBROUTINE gauss_points_2d_p2(mesh)
    ...
    DO m = 1, me
       DO l = 1, l_G
          DO k = 1, k_d
             ...
             ENDDO
          ENDDO
          rjac = ABS(dr(1,1)*dr(2,2) - dr(1,2)*dr(2,1)) !===Sign of Jacobian determinant unknown  
          DO n = 1, n_w
             dw(1, n, l, m)   &
    ...
    DO ms = 1, mes
       ...
       ELSE IF (js(1,ms)==jj(n2,m) .AND. js(2,ms)==jj(n1,m)) THEN
          ...
          sign = -1
       ELSE
          WRITE(*,*) ' BUG : gauss_points_2d_p2, ordering'
          STOP
       END IF
       DO ls = 1, l_Gs
          !write(*,*) wws(:,ls)
       ...
       DO ms = 1, mesh%mi
          DO cote = 1, 2
             ...
             ELSE IF (mesh%jjsi(1,ms)==jj(n2,m) .AND. mesh%jjsi(2,ms)==jj(n1,m)) THEN
                ...
                ngauss(1)=3; ngauss(2)=2; ngauss(3)=1
             ELSE
                WRITE(*,*) ' BUG : gauss_points_2d_p2 ordering, interface'
                STOP
             END IF
             DO ls = 1, l_Gs
