...
CONTAINS
  subroutine p1_p2(aij)
    IMPLICIT NONE
    INTEGER, PARAMETER :: type_fe_p2 = 1, type_fe_p3 = 2 !, nw_p2=(type_fe_p2+2)*(type_fe_p2+1)/2, &
          !, nw_p3=(type_fe_p3+2)*(type_fe_p3+1)/2
    REAL(KIND=8), DIMENSION(:,:)    :: aij 
    INTEGER, DIMENSION(SIZE(aij,2)) :: Cart_FE
    REAL(KIND=8) ::  f1, f2, f3, x, y, delta, one = 1.d0
    REAL(KIND=8), DIMENSION(SIZE(aij,2)) :: xx, yy, xxp, yyp
    INTEGER :: i, j, n, h, k
  ...
  subroutine p2_p3(aij)
    IMPLICIT NONE
    INTEGER, PARAMETER :: type_fe_p2 = 2, type_fe_p3 = 3 !, nw_p2=(type_fe_p2+2)*(type_fe_p2+1)/2, &
          !nw_p3=(type_fe_p3+2)*(type_fe_p3+1)/2
    REAL(KIND=8), DIMENSION(:,:) :: aij 
    INTEGER, DIMENSION(SIZE(aij,2)) :: Cart_FE
