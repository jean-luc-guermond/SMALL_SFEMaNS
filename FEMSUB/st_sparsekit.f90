...
CONTAINS
  ...
  SUBROUTINE st_csr_jr_jc(jjr, jjc, ia, ja)
    !  input coefficient structure of the matrix and
    !  perform the ordering of the unknowns
    !  jj(nodes_per_element, number_of_elements)
    !                  --->  node number in the grid
    IMPLICIT NONE
    INTEGER, DIMENSION(:,:), INTENT(IN)  :: jjr, jjc
    INTEGER, DIMENSION(:),   POINTER     :: ia, ja
    INTEGER :: nparm=180
    INTEGER :: me, nwr, nwc, nmax, npr
    INTEGER :: m, ni, nj, i, j, n_a_d
    INTEGER, DIMENSION(:,:), ALLOCATABLE :: ja_work
    INTEGER, DIMENSION(:),   ALLOCATABLE :: nja, a_d
    nwr = SIZE(jjr, 1)
    nwc = SIZE(jjc, 1)
    me  = SIZE(jjr, 2)
    IF (SIZE(jjr, 2) .NE. SIZE(jjc, 2)) THEN
       WRITE(*,*) 'BUG in st_csr_jr_jc: SIZE(jjr, 2) .NE. SIZE(jjc, 2)'
       STOP
    END IF
    npr = MAXVAL(jjr)
    ALLOCATE (ja_work(npr,nparm), ia(npr+1), a_d(nparm), nja(npr))
    ja_work = 0
    nja = 0
    DO m = 1, me
       DO ni = 1, nwr
          ...
          DO nj = 1, nwc
             ...
             IF (nja(i)==0) THEN
                nja(i) = nja(i) + 1
                ja_work(i,nja(i)) = j
             ELSE IF (MINVAL(ABS(ja_work(i,1:nja(i))-j)) /= 0) THEN 
                nja(i) = nja(i) + 1
                ja_work(i,nja(i)) = j 
             END IF
          END DO
    ...
    END DO
    DEALLOCATE ( ja_work, nja, a_d )
  END SUBROUTINE st_csr_jr_jc

  SUBROUTINE st_csr_jr_jc_block(jjr, nbr, jjc, nbc,  ia, ja)
    !  input coefficient structure of the matrix and
    !  perform the ordering of the unknowns
    !  jj(nodes_per_element, number_of_elements)
    !                  --->  node number in the grid
    ...
    INTEGER                              :: nbr, nbc
    INTEGER :: nparm=180
    INTEGER :: me, nwr, nwc, nmax, npr, npc
    INTEGER :: m, ni, nj, i, j, n_a_d, nptot, ib, jb, ki, kj
    INTEGER, DIMENSION(:,:), ALLOCATABLE :: ja_work
    INTEGER, DIMENSION(:),   ALLOCATABLE :: nja, a_d
    nwr = SIZE(jjr, 1)
    nwc = SIZE(jjc, 1)
    ...
    npc = MAXVAL(jjc)
    nptot = nbr*npr
    ALLOCATE (ja_work(nptot,nparm), ia(nptot+1), a_d(nparm), nja(nptot))
    ja_work = 0
    nja = 0
    DO m = 1, me
       DO ni = 1, nwr
    ...
    END DO
    DEALLOCATE (ja_work,nja,a_d)
  END SUBROUTINE st_csr_jr_jc_block
