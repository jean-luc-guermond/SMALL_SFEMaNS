...
100 FORMAT(3(e11.5,3x),i5)
  ...
  SUBROUTINE plot_scalar_field(jj, rr, uu, file_name)
    ...
    ELSE IF (SIZE(jj,1)==10) THEN
       ...
       ENDDO
    ELSE
       WRITE(*,*) ' Problem in plot_scalar_field ', SIZE(jj,1), SIZE(jj,2)
       STOP
    END IF
    CLOSE(UNIT=unit_w)
    RETURN
...
100 FORMAT(2(e11.5,3x),i5,3x,i5)
  ...
  SUBROUTINE vtk_2d(mesh, champ, unit_file, file_name, field_name)
    ...
    ELSE IF (mesh%gauss%n_w==10) THEN
       WRITE(unit_file,'(A,I7,I8)') 'CELLS ', 9*mesh%me, 9*mesh%me*4
       DO i=1, mesh%me
          WRITE(unit_file,'(A,3(I8,1x))') '3 ',  mesh%jj(1,i)-1,  mesh%jj(8,i)-1,  mesh%jj(6,i)-1
          WRITE(unit_file,'(A,3(I8,1x))') '3 ',  mesh%jj(8,i)-1,  mesh%jj(10,i)-1, mesh%jj(6,i)-1
          WRITE(unit_file,'(A,3(I8,1x))') '3 ',  mesh%jj(8,i)-1,  mesh%jj(9,i)-1,  mesh%jj(10,i)-1
          WRITE(unit_file,'(A,3(I8,1x))') '3 ',  mesh%jj(9,i)-1,  mesh%jj(4,i)-1,  mesh%jj(10,i)-1
          WRITE(unit_file,'(A,3(I8,1x))') '3 ',  mesh%jj(9,i)-1,  mesh%jj(2,i)-1,  mesh%jj(4,i)-1
          WRITE(unit_file,'(A,3(I8,1x))') '3 ',  mesh%jj(6,i)-1,  mesh%jj(10,i)-1, mesh%jj(7,i)-1
          WRITE(unit_file,'(A,3(I8,1x))') '3 ',  mesh%jj(10,i)-1, mesh%jj(5,i)-1,  mesh%jj(7,i)-1
          WRITE(unit_file,'(A,3(I8,1x))') '3 ',  mesh%jj(10,i)-1, mesh%jj(4,i)-1,  mesh%jj(5,i)-1
          WRITE(unit_file,'(A,3(I8,1x))') '3 ',  mesh%jj(7,i)-1,  mesh%jj(5,i)-1,  mesh%jj(3,i)-1
       ENDDO
       WRITE(unit_file,'(A,I7)') 'CELL_TYPES ', 9*mesh%me
       DO i=1, 9*mesh%me
          WRITE(unit_file,'(A)') '5'
       ENDDO
    END IF
    ...
    WRITE(*,*) 'data ...'
    WRITE(unit_file,'(A,I7)') 'POINT_DATA ',mesh%np
    WRITE(unit_file,'(3A)') 'SCALARS ', field_name, ' float 1'
    WRITE(unit_file,'(A)') 'LOOKUP_TABLE default'
    DO i=1, mesh%np
       WRITE(unit_file,'(e13.7,2x)') champ(i)
    ENDDO
